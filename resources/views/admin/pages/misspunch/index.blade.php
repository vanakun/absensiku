...
<div class="container px-6 mx-auto grid">
  <br>
  <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-200">Menunggu Approve Misspunch</h2>
      ...
      <table class="w-full whitespace-no-wrap">
        ...
        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
          ...
          <td class="px-4 py-3">{{ $loop->iteration }}</td>
            ...
            <td class="px-4 py-3 text-sm">{{ $request->user->jam_masuk }}</td>
            <td class="px-4 py-3 text-sm">{{ $request->user->jam_keluar }}</td>
            ...
            <td class="px-4 py-3 text-sm">
              <form action="{{ route('admin.cuti.approvemisspunch', $request->id) }}" method="POST">
                @csrf
                @method('PUT')
            ...
            <td class="px-4 py-3 text-sm">
              <form action="{{ route('admin.cuti.rejectmisspunch', $request->id) }}" method="POST">
                @csrf
                @method('PUT')
...
<div class="container px-6 mx-auto grid">
  <br>
  <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-200">History Approve Misspunch</h2>
  <div class="w-full overflow-hidden rounded-lg shadow-xs mt-4">
    <div class="w-full overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        <thead>
        ...
        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
          ...
          <tr class="text-gray-700 dark:text-gray-400">
            <td class="px-4 py-3">{{ $loop->iteration }}</td>
            <td class="px-4 py-3 text-sm">{{ $request->user->name_lengkap }}</td>
            <td class="px-4 py-3 text-sm">{{ $request->jam_masuk }}</td>
            <td class="px-4 py-3 text-sm">{{ $request->jam_keluar }}</td>
            <td class="px-4 py-3 text-sm">{{ Carbon::parse($request->tgl_presensi)->translatedFormat('l, d F Y') }}</td>
